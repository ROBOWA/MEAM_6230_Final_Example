"""
Surface-Frame Passive DS Impedance Controller with Energy Tank.

Control law (Cartesian torque):
    τ = J^T · D_s · (v_d − v_ee) + g(q)
...
Passivity proof outline (see MEAM_6230 Final Project):
  s_dot = alpha(s)*p_d - beta_t(s,p_t)*p_t - beta_n(s,p_n)*p_n
  v_d   = beta_t'*motion_term + beta_n'*force_term
  F_cart= D_s*(v_d - v_ee)
"""
...
class PolishingControllerTank:
    ...
    def step(self):
        ...
        # Far from surface: use full v_nom so the robot can approach.
        ...
        # ── 11. Map to joint torques ────────────────────────────────────
        tau = J.T @ F_cart

        # ── 12. Gravity + Coriolis + joint-damping compensation ─────────
        tau += self.env.data.qfrc_bias[: self._n_joints]
        tau -= self.env.data.qfrc_passive[: self._n_joints]

        # ── 13. Null-space: joint stiffness/damping toward q_ref ───────
