...
TOOL_Z_OFFSET = 0.279                        # tool tip offset along EE z-axis from link7 [m]

# ── DS parameters ─────────────────────────────────────────────────────
DS_V_TARGET = 0.05    # reaching speed toward surface [m/s]
DS_OMEGA = np.pi / 6  # circular angular frequency [rad/s]
...
CTRL_D_N = 300.0      # normal damping [N·s/m] — steady-state F_n = F_d (paper-style modulation)
...
CTRL_USE_ENERGY_TANK = False   # enable passive energy tank
CTRL_TANK_S0    = 5.0    # initial tank energy [J]
CTRL_TANK_S_MAX = 20.0   # max tank capacity [J]
CTRL_K_ORI = 5     # orientation alignment gain [1/s]: higher value keeps tilt < 2° during approach
...
DISTURBANCE_START_TIME = 30.0                       # [s] into simulation
