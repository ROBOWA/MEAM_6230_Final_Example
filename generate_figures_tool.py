"""Generate report figures for the tool polishing setup (headless, no viewer)."""
import sys, os
...
from sim_env_tool import SimEnvTool
from controller_tool import PolishingControllerTool

# ── Run simulation ────────────────────────────────────────────────────
print("Running tool simulation (headless)...")

C = config.SPHERE_CENTER_TOOL
R = config.SPHERE_RADIUS_TOOL

sphere = SphereSurface(C, R, config.SPHERE_MAX_POLAR)
env    = SimEnvTool(config.SCENE_XML_TOOL)
ds     = PolishingDS(
    sphere,
    r_tool=config.TOOL_RADIUS_TOOL,
...
for k in range(N):
    # normal_force (fn) is the 6th return value — already FT-projected onto n
    v_d, n, sigma, _, _, fn = ctrl.step()
    env.step()
    ee   = env.ee_pos()
    v_ee = env.ee_vel()
    ...
    log_sigma[k]   = sigma
    log_fn[k]      = fn
    log_dist[k]    = sphere.signed_dist(ee, 0.0)   # r_tool=0 for tool setup
...
OUT = os.path.join(os.path.dirname(__file__), "report_figs_tool")
...
ax.set_title("Tool Trajectory during Polishing Phase")
...
# Orbit geometry
_R_eff   = np.sqrt(max(R**2 - config.TOOL_CYL_RADIUS**2, 0.0))
_z_off   = np.sqrt(max(_R_eff**2 - config.DS_R_CIRCLE_TOOL**2, 0.0))
_orbit_z = C[2] + _z_off
# Limit-cycle circle
ax.plot(C[0] + config.DS_R_CIRCLE_TOOL*np.cos(theta),
        C[1] + config.DS_R_CIRCLE_TOOL*np.sin(theta),
        "r-", lw=1.2, label=f"limit cycle  r={config.DS_R_CIRCLE_TOOL*1e3:.0f} mm")
...
_pad = config.DS_R_CIRCLE_TOOL * 1.5
ax.set_xlim(C[0] - _pad, C[0] + _pad)
ax.set_ylim(C[1] - _pad, C[1] + _pad)
ax.set_aspect("equal")
ax.set_xlabel("x [m]"); ax.set_ylabel("y [m]")
ax.set_title(f"Tool Trajectory — Polishing Orbit  (z = {_orbit_z:.3f} m)")
...
# ─── Figure 3: Contact force, distance, sigma ────────────────────────
...
axes[1].set_title("Signed Surface Distance  (+ above, − penetrating)")
axes[1].legend(fontsize=8); axes[1].grid(True, alpha=0.3)

LP = 10
fn_filt = uniform_filter1d(log_fn, size=LP)
axes[2].plot(log_t, log_fn,     lw=0.4, color="tab:orange", alpha=0.3, label="Raw FT $F_n$")
axes[2].plot(log_t, fn_filt,    lw=1.2, color="tab:orange", label="Filtered $F_n$")
axes[2].axhline(config.FORCE_DESIRED_TOOL, color="r", ls="--", lw=1.0,
                label=f"Target $F_d$ = {config.FORCE_DESIRED_TOOL} N")
...
axes[2].set_title("Contact Normal Force (wrist FT sensor)")
...
circ = plt.Circle((C[0], C[1]), config.DS_R_CIRCLE_TOOL, fill=False,
                  color="red", lw=1.5, ls="--",
                  label=f"Limit circle r={config.DS_R_CIRCLE_TOOL} m")
ax.add_patch(circ)
